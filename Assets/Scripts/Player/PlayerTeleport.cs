...
public class PlayerTeleport : MonoBehaviour
{
    ...
    IEnumerator LookForSpace()
    {
        ...
        while (!Input.GetButtonUp("L1"))
        {
            ...
            {
                states.isSqueezedTeleport = Physics2D.OverlapBox(new Vector2(transform.position.x + distance * states.direction, transform.position.y + bc.size.y / 2 - bc.size.x / 2), new Vector2(bc.size.x - 0.015f, bc.size.y - 0.015f), 0f, whatIsPlatform | whatIsDamage);
